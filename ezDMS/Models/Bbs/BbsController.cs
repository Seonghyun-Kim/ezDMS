...
namespace IS_PODS.Controllers
{
    ...
    {   //view는 그냥 이름으로 해주고 기능은 select = get 나머지 =set 붙여주기
        ...
        */
        ...
        public JsonResult setBbsContentDelete(int? bbs_idx) 
        {
            try 
            {
                Mapper.Instance().BeginTransaction();
                ...
                //Mapper.Instace().Delete("Bbs.delBbsFile", new BbsFileModel {bbs_idx = bbs_idx});
        ...
        public JsonResult setBbsReplyDelete(int? bbs_reply_idx, int? parent_reply_idx)
        {
            try
            {
                var isSon = parent_reply_idx != null ? true : false;
                ...
                {
                    Mapper.Instance().Update("Bbs.udtBbsReply", new BbsReplyModel { bbs_reply_content = "삭제된 댓글입니다." });
                    Mapper.Instance().CommitTransaction();
                    return Json(1);
                }
